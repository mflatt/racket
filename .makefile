...
win32-in-place:
	...
	$(MAKE) win32-$(VM)-$(INITIAL_SETUP_MODE)-in-place
	$(MAKE) win32-$(VM)-in-place-setup
...
win32-bc:
	$(MAKE) win32-bc-in-place

bc-in-place:
	$(MAKE) in-place VM=bc

win32-bc-in-place:
	$(MAKE) win32-in-place VM=bc

bc-as-is:
	$(MAKE) as-is VM=bc

win32-bc-as-is:
	$(MAKE) win32-as-is VM=bc
...
win32-cs-base:
	IF "$(RACKET_FOR_BUILD)" == "" $(MAKE) win32-bc-then-cs-base $(WIN32_BOOT_ARGS)
	IF not "$(RACKET_FOR_BUILD)" == "" $(MAKE) win32-just-cs-base SETUP_BOOT_MODE=--chain
...
win32-bc-then-cs-base:
	$(MAKE) win32-bc-base
	$(MAKE) win32-just-cs-base RACKET_FOR_BUILD="$(PLAIN_RACKET)"
...
# Target selector: `win32-base` or `win32-cs-base` or `win32-bc-base`
...
win32-client:
	...
	$(MAKE) win32-distro-build-from-server
	$(MAKE) win32-bundle-from-server
	$(WIN32_RACKET) -l distro-build/set-config $(SET_BUNDLE_CONFIG_q)
	$(MAKE) win32-installer-from-bundle
