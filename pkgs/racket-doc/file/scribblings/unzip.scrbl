...
(if @racket[preserve-timestamps?]) @racket[#f] or a timestamp. The
default @racket[entry-reader] unpacks entries to the filesystem; call
...
is it given, except for permissions on directories on Windows, and it
...
When the resulting returned procedure is called, it will in turn
produce @racket[(void)] unless it is given a hash table as a fourth
argument. When given a hash table, the result is either @racket[#f] or
a thunk. A thunk is returned when arguments refer to a directory and
either a timestamp attribute, permission attribure (Unix and Mac OS
only), or both.
