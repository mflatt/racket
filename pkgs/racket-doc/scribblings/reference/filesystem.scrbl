...
Mac OS. When @racket[#f] and when @racket[dest]s is created, it is
created with @racket[permissions] or the permissions of @racket[src];
however, the process's umask may unset bits in the requested
permissions. Similarly, when @racket[replace-permissions?] is
@racket[#f] and @racket[dest] already exists (and @racket[exists-ok?]
is true), then the permissions of @racket[dest] are left as-is. When
@racket[replace-permissions?] is a true value, then the permissions of
@racket[dest] are set after the file content is copied; meanwhile, if
@racket[dest] is created, then its initial permissions are the same as
if @racket[replace-permissions?] were @racket[#f].

The @racket[exists-ok?/pos] by-position argument can be supplied (for
backward compatibility), or the @racket[exists-ok?] keyword argument
can be supplied, but the @exnraise[exn:fail:contract] if both are
supplied.
