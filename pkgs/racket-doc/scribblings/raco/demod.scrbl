...
dependencies. Submodules are preserved, but their syntax, provides,
...
@itemlist[
 ...
 @item{put all modules to be flattened into an
        @filepath{mine/private/amalgam} subcollection of
        @filepath{mine} (for a specific collection @filepath{mine}),
        where modules within @filepath{mine/private/amalgam} can
        freely refer to each other;}

 @item{create a module @filepath{mine/private/amalgam-src.rkt} that
       requires modules from @filepath{private/amalgam} that need
       to be accessible from outside, where submodules in
       @filepath{mine/private/amalgam-src.rkt} can provided different
 ...
 @item{create a module @filepath{mine/private/amalgam.rkt} as
       ...
       @racketmod[
         ...
         "amalgam-src.rkt"
         #:include (#:collect "mine/private/amalgam")
       ]

       and}

 @item{from outside @filepath{mine/private/amalgam}, use only
       @filepath{mine/private/amalgam.rkt}, perhaps via public modules
       in @filepath{mine} that reprovide from
       @filepath{mine/private/amalgam.rkt}.}
...
syntax objects, provides, and compile-time support (like macros), but
does not insist that all modules are copied into the flattened module.
For example, if a module is referenced by a combination of submodules
...
discards syntax, provides, and compile-time support, so it may be
suitable for flattening a module that implements an end-user program.
