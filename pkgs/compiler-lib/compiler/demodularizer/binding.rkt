...
(define (serialize-binding bind external-path-pos)
  (cond
    [(provided? bind)
     `(#:provided
       ,@(serialize-binding (provided-binding bind) external-path-pos)
    ...
    [else
     ...
     (match (binding-content bind)
       [`(,mod ,sym ,phase ,nom-mod)
        `(#:simple-module-binding #:mpi ,(lookup mod) ,sym ,phase #:mpi ,(lookup nom-mod))]
       [`(,mod ,sym ,phase ,nom-mod ,nom-phase ,nom-sym ,req-phase ,free-id ,insp ,more-noms)
        ;; Currently dropping free-id=? and extra nominals
        `(#:module-binding
          #:mpi ,(lookup mod)
