...
(define (add-import-maps phase-runs names
    ...
    (values
     ...
     (for/list ([r (in-list runs)])
           ...
           (define new-name/import
             (cond
               [(or (not one-m) ; => symbol-named primitive module
                ...
                ;; keep the same name; in the `names` phase, we picked
                ;; a new name to use within the merged module to refer
                ;; to that import
                (import new-name
                        use
                        ext-name)]
               [else
                ...
                (cond
                  [(hash-ref excluded-module-mpis import-path/submod #f)
                   ...
                   (import new-name use src-new-name)]
                  [else
                   ;; Merged here, so directly use the new source name, forgetting
                   ;; the name be picked specifically for the import in this linklet
                   (find-name names (cons import-path/submod phase-level) src-int-name)])]))
       ...
       (for* ([(name i) (in-hash import-map)])
         (if (import? i)
             (log-demodularizer-debug "    ~a -> ~a ~a ~a" name
