...
(struct doc (category? category priority path spec))
  ...
  (let* ([custom-secs (make-hash)]
    ...
    (define (contents renderer part resolve-info)
          ...
          (cond
            ...
            [else
             ...
             (define props (cond
                          [(hash? props*) props*]
                          [else #hasheq()]))
             (define lang (let ([l (hash-ref props 'language-family #f)])
             ...
             (define cat+priority (let* ([ht* (hash-ref props 'category #f)]
                                    ...
                                    (or (hash-ref ht main-language-family #f)
                                        (and (or (not (doc-category? d))
                                                 (not (equal? main-language-family "Racket")))
                                             (hash-ref ht #'default #f))
                                        (list (if (equal? main-language-family "Racket")
                                                  (racketize-category (doc-category d))
                                                  (rename-category (car lang) (doc-category d)))
                                              (doc-priority d)))))
             (define cat (or (and (pair? cat+priority)
                                  (let ([s (car cat+priority)])
                                    (and (or (symbol? s) (string? s) (category? s))
                                         s)))
                             (if (equal? main-language-family "Racket")
                                 (racketize-category 'library)
                                 (rename-category (car lang) 'library))))
      ...
      (define sections+custom
        (append-map (λ (sec)
                      (if (eq? 'other-library (sec-cat sec))
                          (append (for/list ([label (in-list
                                                     (sort (hash-keys custom-secs)
                                                           (lambda (a b)
                                                             (cond
                    ...
                    (if (equal? main-language-family "Racket")
                        sections
                        ;; move 'racket-core and 'teaching to before 'other-library
                        (let loop ([l sections])
                          (define (find name)
                            (ormap (lambda (s) (and (eq? (sec-cat s) name) s)) sections))
                          (cond
                            [(null? l) null]
                            [(eq? (sec-cat (car l)) 'other-library)
                             (list* (find 'racket-core)
                                    (find 'language)
                                    (find 'teaching)
                                    (cons (car l)
                                          (loop (cdr l))))]
