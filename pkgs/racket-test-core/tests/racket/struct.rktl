...
(let-values ([(prop:p p? p-ref) (make-struct-type-property 'prop (lambda (x y) (add1 x)))]
  ...
  (let-values ([(struct:klass make-klass klass? klass-ref) (make-struct-type-type 'klass #f 2)])
    (define (ok-klass? v)
      (and (klass? v)
    ...
    (let ()
      ...
      (test #t struct-type-type? struct:klass)
      (let ()
        ...
        (test #f struct-type-type? meta-a)
        (test #f struct-type-type? meta-b)
        (test #f klass? meta-a)
        (test #t klass? meta-b)
        (err/rt-test (make-struct-type-type 'klass2 s:a 1))
        (err/rt-test (make-struct-type-type 'klass2 s:b 1))
      ...
      (let-values ([(struct:klass2 make-klass2 klass2? klass2-ref) (make-struct-type-type 'klass2 struct:klass 1)])
        (define (ok-klass2? v)
          (and (klass? v)
               (klass2? v)
               (eq? (klass-ref v 0) 'one)
        ...
        (let ()
          ...
          (test #t struct-type-type? struct:klass2))))))
