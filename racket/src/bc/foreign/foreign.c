...
/* data := {name, c-function, itypes, otype, cif} */
{
   ...
   * ignore ivals. Things get complicated if the C call can involve GC, usually due to a
   * Racket callback; the programmer will need to arrange for pointers
   * that cannot move, bit it's a pain to arrange for the pointers to be retained
   * if they're generated by a converting ctype, so we keep avalues live
   * during the foreign call via keep_avalues. Because of all this, the *only*
   * arrays that should not be ignored by the GC are avalues and keep_avalues.
   */
  ForeignAny *ivals;
  void **avalues, **keep_avalues, *p, *newp;
  GC_CAN_IGNORE ForeignAny stack_ivals[MAX_QUICK_ARGS], oval;
  void *stack_avalues[MAX_QUICK_ARGS];
  void *stack_keep_avalues[MAX_QUICK_ARGS];
  ...
  if ((nargs <= MAX_QUICK_ARGS)) {
    ...
    keep_avalues = stack_keep_avalues;
    ...
    keep_avalues = scheme_malloc(nargs * sizeof(void*));
  ...
  for (i=0; i<nargs; i++) { keep_avalues[i] = NULL; } /* no need for these refs */
  avalues = NULL;
  keep_avalues = NULL;
