...
/* Registers with rktio that an operating-system signal handler is
   being modified within the process but outside of rktio, where
   `sig_id` is a signal identifier --- such as SIGINT or SIGTERM. This
   notification allows rktio to record that the signal should be
   restored after forking a new Unix process. (Without this
   information, rktio would have to know the range of possible signal
   IDs on every platform.) Signal registrations should happen only
   before multiple threads use rktio, and registration of the signal
   can happen before any `rktio_init` call. */
