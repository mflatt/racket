(library (linklet)
  ...
  (define (call-getting-more-paths proc)
    (let ([more-paths '()])
      (proc (lambda (v)
              (and (path? v)
                   (begin
                     (set! more-paths (cons v more-paths))
                     #t))))
      (list->vector (reverse more-paths))))
  ...
  ;; returns code bytevector and more-paths vector
  (define (compile*-to-bytevector s)
    (let-values ([(o get) (open-bytevector-output-port)])
      (let ([more-paths (compile-to-port* (list `(lambda () ,s)) o)])
        (values (get) more-paths))))
  ...
  (define (compile-to-bytevector s format)
    (cond
      [(eq? format 'interpret)
       (let-values ([(o get) (open-bytevector-output-port)])
         (let ([more-paths (fasl-write-code* s o)])
           (values (get) more-paths)))]
  ...
  (define (eval-from-bytevector bv paths more-paths format)
    (add-performance-memory! 'faslin-code (bytevector-length bv))
    (cond
      [(eq? format 'interpret)
       (let ([r (performance-region
                 'faslin-code
                 (fasl-read (open-bytevector-input-port bv) 'load more-paths))])
         (performance-region
          'outer
      ...
      [else
       (let ([proc (performance-region
                    'faslin-code
                    (code-from-bytevector bv more-paths))])
         (if (null? paths)
             proc
             (#%apply proc paths)))]))

  (define (code-from-bytevector bv more-paths)
    (let ([i (open-bytevector-input-port bv)])
      (let ([r (load-compiled-from-port i more-paths)])
        (performance-region
         'outer
         (r)))))
    ...
    (let ([f (wrapped-code-content wc)])
      (if (procedure? f)
          ...
          (performance-region
           ...
           (cond
             [(bytevector? f)
              (let* ([f (code-from-bytevector f (wrapped-code-more-paths wc))])
  ...
  (define-record-type linklet
    (fields (mutable code) ; the procedure or interpretable form
            ...
            more-paths     ; vector of additional source-location paths intercepted during fasl
  ...
  (define (set-linklet-paths linklet paths more-paths)
                                           ...
                                           (lambda (expr arity-mask name)
                                             (performance-region
                                              ...
                                              (let ([expr (show lambda-on? "lambda" (correlated->annotation expr serializable? sfd-cache))])
                                                (if serializable?
                                                    (let-values ([(code more-paths) (if cross-machine
                                                                                        (cross-compile cross-machine expr)
                                                                                        (compile*-to-bytevector expr))])
                                                      (make-wrapped-code code more-paths arity-mask (extract-inferred-name expr name)))
                                                    (compile* expr)))))])))]))
