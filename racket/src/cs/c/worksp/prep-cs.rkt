...
(define machine-name (if (= 32 (system-type 'word))
			 "ti3nt"
			 "ta6nt"))
(define cs-dir (build-path 'up 'up 'up "build" "ChezScheme"))

(command-line
 #:once-each
 [("--machine") mach "Select a machine name"
  (set! machine-name mach)]
 [("--scheme") dir "Select a Chez Scheme build directory"
  (set! cs-dir dir)]
 #:args ()
 (void))
