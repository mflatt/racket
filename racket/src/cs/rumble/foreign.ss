  ...
  (cpointer-fptr-set! p (let ([m (cpointer+offset-base-fptr p)])
                          (cond
                            [(ftype-object-pointer? m)
                             (make-ftype-object-pointer (ftype-pointer-object m)
                                                        n)]
                            [else
...
(define (do-ptr-add p n save-tags?)
  (cond
    ...
    [(bytes? p)
     (make-cpointer+offset (make-ftype-object-pointer p n)
                           #f
                           (make-ftype-object-pointer p))]
    [(not p)
                ...
                (lambda (for-whom dest-c offset s)
                  (let ([m (cptr->fptr for-whom dest-c)])
                    (if (and (ftype-object-pointer? m)
                             (reference-bytevector? (ftype-pointer-object m)))
...
(define-ctype/promote-pointer _bytes 'bytes
  ...
  (lambda (for-whom x)
    (cond
      [(not x) null-fptr]
      [(bytes? x) (make-ftype-object-pointer x)]
...
(define pointer-set!
  ...
  (lambda (for-whom dest-c offset s)
    (let ([m (cptr->fptr for-whom dest-c)]
      ...
      (if (and (ftype-object-pointer? m)
               (ftype-object-pointer? v)
               (reference-bytevector? (ftype-pointer-object m))
               (eqv? (ftype-pointer-offset v) 0))
          ;; use `bytevector-reference-set!` to get write barrier
          (let ([offset (+ offset (ftype-pointer-offset m))])
            (bytevector-reference-set! (ftype-pointer-object m) offset (ftype-pointer-object v)))
...
(define (normalized-malloc size mode)
  ...
  (cond
   ...
   [(eq? mode 'atomic-interior)
    ...
    (fptr->cptr (make-ftype-object-pointer (make-immobile-bytevector size)))]
   [(eq? mode 'interior)
    ;; Ditto
    (fptr->cptr (make-ftype-object-pointer (make-immobile-reference-bytevector size)))]
   [(eq? mode 'zeroed-atomic)
    (fptr->cptr (make-ftype-object-pointer (make-bytevector size 0)))]
   [(eq? mode 'zeroed-atomic-interior)
    (fptr->cptr (make-ftype-object-pointer (make-immobile-bytevector size 0)))]
   [else
...
(define (malloc-immobile-cell v)
  (let ([vec (make-immobile-reference-bytevector (foreign-sizeof 'ptr))])
    ...
    (fptr->cptr (make-ftype-object-pointer vec))))

(define/who (free-immobile-cell b)
  (with-global-lock
   (eq-hashtable-delete! immobile-cells (ftype-pointer-object (cptr->fptr who b)))))

(define/who (immobile-cell-ref b)
  (bytevector-reference-ref  (ftype-pointer-object (cptr->fptr who b)) 0))

(define (immobile-cell->address b)
  (ftype-pointer-address (cpointer-fptr b)))

(define (address->immobile-cell a)
  (fptr->cptr (make-ftype-object-pointer (reference-address->object a))))
...
(define (flvector->cpointer flvec)
  (fptr->cptr (make-ftype-object-pointer flvec)))
        ...
        (let ([arity-mask (bitwise-arithmetic-shift-left 1 (length in-types))])
          (lambda (to-wrap)
            (let* ([proc-p (cptr->fptr 'ffi-call to-wrap)]
                   [proc (and (not (ftype-object-pointer? proc-p))
  ...
  (struct-set-equal+hash! (record-type-descriptor cpointer)
                          ...
                          (lambda (a hc)
                            (let ([m (cpointer-fptr a)])
                              (if (ftype-object-pointer? m)
                                  (+ (eq-hash-code (ftype-pointer-object m))
                                     (ftype-pointer-offset m))
