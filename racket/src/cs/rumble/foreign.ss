...
(meta define (build-pointer-gensym-id name gcable?)
      (let ([name-str (#%string-append (#%symbol->string name)
                                       (if gcable?
                                           "/gcable"
                                           ""))])
        (#%datum->syntax
                          ...
                          (cond
                            ...
                            [else
                             ...
                             (with-syntax ([id (#%datum->syntax
                                           ...
                                           [uid (if for-struct?
                                                    #'#f
                                                    (build-pointer-gensym-id name (eq? (syntax->datum type-stx)
                                                                                       'ftype-scheme-object-pointer)))]
                                           [type type-stx])
...
(define-syntax (ffi2-ptr?-maker stx)
  (syntax-case stx ()
    [(_ kind tag)
     (with-syntax ([uid (build-pointer-gensym-id (datum tag) (eq? (datum kind) 'pointer/gc))]
                   [base-type (if (eq? (datum kind) 'pointer/gc)
                                  #'ftype-scheme-object-pointer
                                  #'ftype-pointer)])
       #'(let ()
           (define-ftype tagged base-type (nongenerative uid))
