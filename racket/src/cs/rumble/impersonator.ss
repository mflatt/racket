...
;; `orig` is an impersonator, 
(define (impersonate-struct-property-ref val key orig)
  (let loop ([v orig])
    (cond
      [(not (impersonator? v)) val]
      [(struct-impersonator? v)
       (let ([wrapper (imp-procs-ref1 (struct-impersonator-procs v) key)])
         (cond
           [(pair? wrapper)
           ...
           [else
            (loop (impersonator-next v))]))]
      [(struct-chaperone? v)
       (let ([wrapper (imp-procs-ref1 (struct-chaperone-procs v) key)])
         (cond
           [wrapper
            (let* ([r (cond
                        [(pair? wrapper)
