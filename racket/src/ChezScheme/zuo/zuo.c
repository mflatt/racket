...
typedef struct zuo_t {
  int tag;
  /* but every subtype must have more to make it at least as
...
#define ZUO_STRING_SIZE(len) (((sizeof(zuo_string_t) + (len) + 1 + 3) >> 2) << 2)
...
static zuo_int_t heap_size = (8*1024*1024);
...
static zuo_int_t object_size(zuo_t *obj) {
  ...
  case zuo_string_tag:
    return ZUO_STRING_SIZE(((zuo_string_t *)obj)->len);
  case zuo_cons_tag:
...
static zuo_t *zuo_sized_string(const char *str, zuo_int_t len) {
  zuo_string_t *obj = (zuo_string_t *)zuo_new(zuo_string_tag, ZUO_STRING_SIZE(len));
  obj->len = len;
...
zuo_t *zuo_trie_lookup(zuo_t *trie, zuo_t *sym) {
  zuo_int_t id = ((zuo_symbol_t *)sym)->id;
...
void zuo_trie_set(zuo_t *trie, zuo_t *sym, zuo_t *val) {
  zuo_int_t id = ((zuo_symbol_t *)sym)->id;
  while (id > 0) {
...
zuo_t *trie_extend(zuo_t *trie, zuo_int_t id, zuo_t *val) {
  ...
  if (id > 0) {
    ...
    new_trie->next[i] = trie_extend(((zuo_trie_node_t *)trie)->next[i], id >> ZUO_TRIE_BFACTOR_BITS, val);
...
zuo_t *zuo_trie_extend(zuo_t *trie, zuo_t *sym, zuo_t *val) {
  return trie_extend(trie, ((zuo_symbol_t *)sym)->id, val);
}
...
static void zuo_out(zuo_out_t *out, zuo_t *obj, int depth, zuo_print_mode_t mode) {
  if (obj == zuo_undefined)
    ...
    zuo_int_t i = ((zuo_integer_t *)obj)->i, n, w;
    if (i < 0) {
      out_char(out, '-');
      i = -i;
    }
...
static zuo_t *zuo_in(const unsigned char *s, zuo_int_t *_o, int depth) {
  ...
  if (c == 0)
    ...
    (*_o)++;
    n = zuo_in(s, _o, depth);
    return zuo_integer(-((zuo_integer_t *)n)->i);
  } else if (c == '\'') {
    zuo_t *v;
    (*_o)++;
...
static zuo_t *zuo_read_one(zuo_t *obj) {
  ...
  return zuo_read_one_str((char *)&((zuo_string_t *)obj)->s);
}
...
static zuo_t *zuo_read_all(zuo_t *obj) {
  ...
  return zuo_read_all_str((char *)&((zuo_string_t *)obj)->s);
}
...
static char *zuo_string_to_c(zuo_t *obj) {
  ...
  len = ((zuo_string_t *)obj)->len;
  s = malloc(len + 1);
  memcpy(s, &((zuo_string_t *)obj)->s, len);
  s[len] = 0;
...
static zuo_t *call_primitive0(zuo_t *data, zuo_t *args) {
  return ((zuo_t *(*)())((void *)((zuo_integer_t *)data)->i))();
}

static zuo_t *zuo_primitive0(zuo_t *(*f)(), const char *name) {
  return zuo_primitive(call_primitive0, zuo_integer((zuo_int_t)f), (1 << 0), zuo_string(name));
}

static zuo_t *call_primitive1(zuo_t *data, zuo_t *args) {
  return ((zuo_t *(*)(zuo_t *))((void *)((zuo_integer_t *)data)->i))(ZUO_CAR(args));
}

static zuo_t *zuo_primitive1(zuo_t *(*f)(zuo_t *), const char *name) {
  return zuo_primitive(call_primitive1, zuo_integer((zuo_int_t)f), (1 << 1), zuo_string(name));
}

static zuo_t *call_primitive2(zuo_t *data, zuo_t *args) {
  return ((zuo_t *(*)(zuo_t *, zuo_t *))((void *)((zuo_integer_t *)data)->i))(ZUO_CAR(args), ZUO_CAR(ZUO_CDR(args)));
}

static zuo_t *zuo_primitive2(zuo_t *(*f)(zuo_t *, zuo_t *), const char *name) {
  return zuo_primitive(call_primitive2, zuo_integer((zuo_int_t)f), (1 << 2), zuo_string(name));
}

static zuo_t *call_primitive3(zuo_t *data, zuo_t *args) {
  return ((zuo_t *(*)(zuo_t *, zuo_t *, zuo_t *))((void *)((zuo_integer_t *)data)->i))(ZUO_CAR(args),
                                                                                       ZUO_CAR(ZUO_CDR(args)),
                                                                                       ZUO_CAR(ZUO_CDR(ZUO_CDR(args))));
}

static zuo_t *zuo_primitive3(zuo_t *(*f)(zuo_t *, zuo_t *, zuo_t *), const char *name) {
  return zuo_primitive(call_primitive3, zuo_integer((zuo_int_t)f), (1 << 3), zuo_string(name));
}

static zuo_t *call_primitiveN(zuo_t *data, zuo_t *args) {
  return ((zuo_t *(*)(zuo_t *))((void *)((zuo_integer_t *)data)->i))(args);
}

static zuo_t *zuo_primitiveN(zuo_t *(*f)(zuo_t *), const char *name) {
  return zuo_primitive(call_primitiveN, zuo_integer((zuo_int_t)f), -1, zuo_string(name));
}
...
static zuo_t *zuo_string_length(zuo_t *obj) {
  ...
  return zuo_integer(((zuo_string_t *)obj)->len);
}
...
static zuo_t *zuo_string_ref(zuo_t *obj, zuo_t *i) {
  ...
  idx = ((zuo_integer_t *)i)->i;
  if ((idx < 0) || (idx >= ((zuo_string_t *)obj)->len))
...
static zuo_t *zuo_substring(zuo_t *obj, zuo_t *start_i, zuo_t *end_i) {
  ...
  s_idx = ((zuo_integer_t *)start_i)->i;
  e_idx = ((zuo_integer_t *)end_i)->i;
  len = ((zuo_string_t *)obj)->len;
...
static zuo_t *zuo_string_to_symbol(zuo_t *obj) {
  ...
  return zuo_symbol((const char *)&((zuo_string_t *)obj)->s);
}
...
static zuo_t *zuo_hash_ref(zuo_t *ht, zuo_t *sym, zuo_t *defval) {
  zuo_t *v;
  if (ht->tag != zuo_trie_node_tag)
    zuo_fail1w("hash-ref", "not a hash table", ht);
  if (sym->tag != zuo_symbol_tag)
    zuo_fail1w("hash-ref", "not a symbol", sym);
...
static zuo_t *zuo_hash_set(zuo_t *ht, zuo_t *sym, zuo_t *val) {
  if (ht->tag != zuo_trie_node_tag)
    zuo_fail1w("hash-set", "not a hash table", ht);
  if (sym->tag != zuo_symbol_tag)
    zuo_fail1w("hash-set", "not a symbol", sym);
...
static zuo_t *zuo_string_eql(zuo_t *n, zuo_t *m) {
  ...
  check_string(who, n);
  check_string(who, m);
  return (((((zuo_string_t *)n)->len == ((zuo_string_t *)m)->len)
           && !memcmp(&((zuo_string_t *)n)->s, &((zuo_string_t *)m)->s, ((zuo_string_t *)n)->len))
          ? zuo_true
          : zuo_false);
}
...
static zuo_t *zuo_make_mapped_symbols() {
  return zuo_prim_symbols;
}
...
static void interp_step() {
  ...
  if (e->tag == zuo_symbol_tag) {
    ...
    if (rator == zuo_quote_symbol) {
      zuo_interp_v = zuo_car(zuo_cdr(e));
    } else if (rator == zuo_if_symbol) {
      zuo_t *d = zuo_cdr(e);
      zuo_interp_e = zuo_car(d);
      zuo_interp_k = zuo_cont(zuo_if_cont,
                              zuo_cdr(d), zuo_interp_env,
                              zuo_interp_k);
    } else if (rator == zuo_lambda_symbol) {
      zuo_interp_v = zuo_closure(zuo_interp_e, zuo_interp_env);
    } else if (rator == zuo_let_symbol) {
      zuo_t *d = zuo_cdr(e);
      zuo_interp_e = zuo_car(zuo_cdr(zuo_car(zuo_car(d))));
      ...
      zuo_interp_k = zuo_cont(zuo_apply_cont,
                              zuo_cons(zuo_null, zuo_cdr(e)), zuo_interp_env,
                              zuo_interp_k);
    }
  } else
    zuo_interp_v = e;
...
static void continue_step() {
  ...
  case zuo_apply_cont:
    {
      zuo_t *vals = zuo_car(k->data);
      zuo_t *exps = zuo_cdr(k->data);
      ...
      if (exps == zuo_null) {
        ...
        if (rator->tag == zuo_closure_tag) {
          ...
          zuo_t *env = f->env;
          zuo_t *args = zuo_car(zuo_cdr(f->lambda));
          zuo_t *body = zuo_cdr(zuo_cdr(f->lambda));
          zuo_t *body_d = zuo_cdr(body);
          if (body_d != zuo_null)
          ...
          while (args->tag == zuo_cons_tag) {
            if (vals == zuo_null)
              break;
            env = env_extend(env, zuo_car(args), zuo_car(vals));
            args = zuo_cdr(args);
            vals = zuo_cdr(vals);
          }
          ...
          zuo_interp_e = zuo_car(body);
          zuo_interp_env = env;
          zuo_interp_v = zuo_undefined;
        } else if (rator->tag == zuo_primitive_tag) {
          zuo_primitive_t *f = (zuo_primitive_t *)rator;
          zuo_int_t n = ((f->arity_mask == -1) ? 0 : zuo_length_int(zuo_cdr(vals)));
          if (f->arity_mask & (1 << ((n > 10) ? 10 : n)))
            zuo_interp_v = f->proc(f->data, zuo_cdr(vals));
          else
            zuo_fail1("wrong argument count", vals);
        } else
          zuo_fail1("not a function for call", rator);
      } else {
        zuo_interp_e = zuo_car(exps);
        zuo_interp_env = k->env;
        zuo_interp_k = zuo_cont(zuo_apply_cont, zuo_cons(vals, zuo_cdr(exps)), zuo_interp_env, zuo_interp_k);
        zuo_interp_v = zuo_undefined;
      }
    }
    break;
  case zuo_let_cont:
    zuo_interp_e = zuo_car(zuo_cdr(k->data));
    zuo_interp_env = env_extend(k->env, zuo_car(zuo_car(zuo_car(k->data))), zuo_interp_v);
    zuo_interp_v = zuo_undefined;
    break;
  case zuo_begin_cont:
    {
      zuo_t *d = zuo_cdr(k->data);
      zuo_interp_e = zuo_car(k->data);
      zuo_interp_env = k->env;
      if (d != zuo_null)
        zuo_interp_k = zuo_cont(zuo_begin_cont,
                                d, zuo_interp_env,
...
static void check_path_string(const char *who, zuo_t *obj) {
  ...
  if ((obj->tag != zuo_string_tag)
      || ((zuo_string_t *)obj)->len == 0)
    zuo_fail1w(who, msg, obj);

  for (i = ((zuo_string_t *)obj)->len; i--; ) {
    if (((zuo_string_t *)obj)->s[i] == 0)
      zuo_fail1w(who, msg, obj);
  }
}

int path_is_complete(zuo_t *obj) {
#ifdef WIN32
  FIXME;
#else
  return (((zuo_string_t *)obj)->s[0] == '/');
#endif
}
...
static zuo_t *zuo_build_path(zuo_t *pre, zuo_t *post) {
  ...
  len += ((zuo_string_t *)post)->len;

  path = (zuo_string_t *)zuo_new(zuo_string_tag, ZUO_STRING_SIZE(len));
  path->len = len;
  path->s[len] = 0;
  len = ((zuo_string_t *)pre)->len;
  memcpy(&path->s, &((zuo_string_t *)pre)->s, len);
  if (add_sep)
    path->s[len++] = '/';
  memcpy(&path->s[len], &((zuo_string_t *)post)->s, ((zuo_string_t *)post)->len);
...
static zuo_t *zuo_require(zuo_t *module_path) {
...

  {
    ...
    filename = (char *)&((zuo_string_t *)module_path)->s;
    in = fopen(filename, "r");
    if (in == NULL)
...
zuo_t *zuo_process(zuo_t *command_and_args)
{
  ...
  return zuo_cons(zuo_integer(pid),
                  zuo_cons(zuo_integer(in),
                           zuo_cons(zuo_integer(out),
                                    zuo_cons(zuo_integer(err),
                                             zuo_null))));
}
...
#define TRIE_SET_AND_SAVE_NAME(name, prim)              \
  do {                                                  \
    zuo_t *sym = zuo_symbol(name);                      \
    zuo_prim_symbols = zuo_cons(sym, zuo_prim_symbols); \
    zuo_trie_set(zuo_top_env, sym, prim);               \
  } while (0)

#define ZUO_TOP_ENV_SET_PRIMITIVE0(name, proc) \
  TRIE_SET_AND_SAVE_NAME(name, zuo_primitive0(proc, name))
#define ZUO_TOP_ENV_SET_PRIMITIVE1(name, proc) \
  TRIE_SET_AND_SAVE_NAME(name, zuo_primitive1(proc, name))
#define ZUO_TOP_ENV_SET_PRIMITIVE2(name, proc) \
  TRIE_SET_AND_SAVE_NAME(name, zuo_primitive2(proc, name))
#define ZUO_TOP_ENV_SET_PRIMITIVE3(name, proc) \
  TRIE_SET_AND_SAVE_NAME(name, zuo_primitive3(proc, name))
#define ZUO_TOP_ENV_SET_PRIMITIVEN(name, proc) \
  TRIE_SET_AND_SAVE_NAME(name, zuo_primitiveN(proc, name))
...
int main(int argc, char **argv) {
  ...
  zuo_library_path = zuo_build_path(zuo_current_directory(), zuo_string("lib"));

  zuo_prim_symbols = zuo_null;

  ZUO_TOP_ENV_SET_PRIMITIVE0("void", zuo_make_void);
  ZUO_TOP_ENV_SET_PRIMITIVE0("namespace-mapped-symbols", zuo_make_mapped_symbols);
