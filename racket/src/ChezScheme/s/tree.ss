(module (empty-tree full-tree tree-extract tree-for-each tree-fold-left tree-bit-set? tree-bit-set tree-bit-unset
                    tree-bit-count tree-bit-length tree-same? tree-merge
                    tree-simplify-for-readonly tree-small-as-bignum
  ...
  ;; If a tree is small enough, then a bignum is likely better
  ;; for a readonly form
  (define tree-small-as-bignum
    (lambda (st size)
      (and (pair? st)
           (< size (* 6 (fixnum-width)))
           (let convert ([st st] [size size])
             (cond
               [(fixnum? st) st]
               [(eq? st full-tree)
                (sub1 (bitwise-arithmetic-shift-left 1 size))]
               [else
                (let ([split (compute-split size)])
                  (+ (convert (tree-node-left st) split)
                     (bitwise-arithmetic-shift-left (convert (tree-node-right st) (- size split))
                                                    split)))])))))
...
(define $make-livemask
  (lambda (size t intern-table)
    (let ([t (or (tree-simplify-for-readonly t size)
                 (tree-small-as-bignum t size)
                 t)])
      (if (number? t)
          t
          (let ([t (cons size t)])
            (or (hashtable-ref intern-table t #f)
                (begin
                  (hashtable-set! intern-table t t)
                  t)))))))

(define $livemask-size
  (lambda (livemask)
    (if (number? livemask)
        (integer-length livemask)
        (tree-bit-length (cdr livemask) (car livemask)))))

(define $livemask-member?
