...
(ftype-pointer-offset fptr) [procedure]
...
(let ()
    ...
    (case-lambda
      ...
      [(r ftype error?)
       (cond
         ...
         [(eq? (syntax->datum ftype) 'ftype-object-pointer)
      ...
      (syntax-case ftype ()
        [(*/&-kwd ftype-name . ptr-kind)
         (and (or (eq? (datum */&-kwd) '*)
              ...
              (member (datum ptr-kind) '(() (ftype-pointer) (ftype-object-pointer))))
           ...
           (if (eq? (datum */&-kwd) '*)
               ...
               (cond
                 ...
                 [(null? (datum ptr-kind))
                  (cons ftd ftd)]
                 [else
                  (let ([rep-fptd (if (eq? (car (datum ptr-kind)) 'ftype-object-pointer)
                                      rtd/object-fptr
                                      rtd/fptr)])
                    (cons ftd
  ...
  (set-who! ftype-pointer-code
    (lambda (fptr)
      (unless ($code-fptr? fptr) ($oops who "~s is not an ftype code pointer" fptr))
      (#3%ftype-pointer-code fptr)))
