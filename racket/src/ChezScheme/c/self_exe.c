/*
  ...
  the `argv0` argument; in particular, it is not used on Windows or
  macOS. At worst, on Unix, uses `PATH` to convert `exec_file` into a
  path.

  The result type `self_exe_t` is normally `char*`, but it's `wchar_t`
  on Windows --- unless `SELF_EXE_WINDOWS_AS_UTF8` is defined, in
  which case it's `char*` and the path is encoded as UTF-8. In all
  cases, the result is `malloc`ed, so it can be `free`d.
...
#if defined(WIN32) && !defined(SELF_EXE_WINDOWS_AS_UTF8)
...
char *get_process_executable_path(const char *exec_file)
{
  char *s;
...
# endif
char *GENERIC_GET_SELF_PATH_NAME(const char *exec_file)
{
