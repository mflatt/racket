...
;; The `in` argument is an input port.
;; The `vm-bytes` argument describes the virtual machine.
;; The `hash->linklet-directory` and `hash->linklet-bundle`
;; functions implement the same functionality as from `#%linklet`.
;; The `read-linklet-bundle-hash` function takes a byte string
;; and extracts the hash-table content for a linklet bundle ---
;; including, of course, the linklet in the bundle.
  ...
  (define (read-linklet-or-directory initial?)
    ...
    (cond
      [(eqv? tag (char->integer #\B))
       ...
       (define bstr (read-bytes len in))
       (define b-ht (read-linklet-bundle-hash bstr))
       (hash->linklet-bundle
