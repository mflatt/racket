...
@racket['keep-mode] to a true value, the mode is explicitly applied again
to @racket[destination]---whether @racket[destination] was just
created and exists already, and ignoring the process's umask. On
...
The @racket[:keep-mode] hash table maps
@racket['keep-mode] to @racket[#true].
