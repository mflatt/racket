(module for '#%kernel
  ...
  (define-syntax (for/foldX/derived stx)
    (syntax-case stx ()
      ...
      [(_ [orig-stx inner-recur nested? emit? ()] ([int-var fold-var] ...) next-k break-k final?-id ()
          expr1 expr ...)
      ...
      ;; Emit case:
      [(_ [orig-stx inner-recur nested? #t binds] ([int-var fold-var] ...) next-k break-k final?-id rest expr1 . body)
       (with-syntax ([(([outer-binding ...]
                        outer-check
                        [loop-binding ...]
                        pos-guard
      ...
      [(_ [orig-stx inner-recur nested? emit? binds] fold-bind next-k break-k final?-id (#:splice form . rest) . body)
       (with-syntax ([(id clause ...) (expand-splicing-clause #'orig-stx #'form)])
         (syntax-property #'(for/foldX/derived [orig-stx inner-recur nested? emit? binds]
                              fold-bind next-k break-k final?-id (clause ... #:when #t . rest) . body)
                          'disappeared-use
                          (syntax-local-introduce #'id)))]
      ;; Guard case, no pending emits:
      [(_ [orig-stx inner-recur nested? #f ()] fold-bind next-k break-k final?-id (#:when expr . rest) . body)
       #'(if expr
             (for/foldX/derived [orig-stx inner-recur nested? #f ()]
               fold-bind next-k break-k final?-id rest . body)
             next-k)]
      ;; Negative guard case, no pending emits:
      [(_ [orig-stx inner-recur nested? #f ()] fold-bind next-k break-k final?-id (#:unless expr . rest) . body)
       #'(if expr
             (if final?-id break-k next-k)
             (for/foldX/derived [orig-stx inner-recur nested? #f ()]
               fold-bind next-k break-k final?-id rest . body))]
      ;; Break case, no pending emits:
      [(_ [orig-stx inner-recur nested? #f ()] fold-bind next-k break-k final?-id (#:break expr . rest) . body)
       #'(if expr
             break-k
             (for/foldX/derived [orig-stx inner-recur nested? #f ()]
               fold-bind next-k break-k final?-id rest . body))]
      ;; Final case, no pending emits:
      [(_ [orig-stx inner-recur nested? #f ()] fold-bind next-k break-k final?-id (#:final expr . rest) . body)
       #'(let ([final? (or expr final?-id)])
           (for/foldX/derived [orig-stx inner-recur nested? #f ()]
             fold-bind next-k break-k final? rest . body))]
      ;; General "do" case, no pending emits:
      [(_ [orig-stx inner-recur nested? #f ()] fold-bind next-k break-k final?-id (#:do forms . rest) . body)
       (syntax-case #'forms ()
         [(form ...)
          #'(let ()
              form ...
      ...
      [(frm [orig-stx inner-recur nested? #f binds] fold-bind next-k break-k final?-id (kw expr . rest) . body)
       (or (eq? (syntax-e #'kw) '#:when)
           (eq? (syntax-e #'kw) '#:unless)
           (eq? (syntax-e #'kw) '#:break)
           (eq? (syntax-e #'kw) '#:final)
           (eq? (syntax-e #'kw) '#:do))
       #'(frm [orig-stx inner-recur nested? #t binds] fold-bind next-k break-k final?-id (kw expr . rest) . body)]
      ...
      [(frm [orig-stx inner-recur nested? #f binds] fold-bind next-k break-k final?-id (clause . rest) . body)
       (with-syntax ([bind (expand-clause #'orig-stx #'clause)])
         (let ([r #`(frm [orig-stx inner-recur nested? nested? (bind . binds)]
           ...
           (if d
               ...
               r)))]
      [(_ [orig-stx . _] fold-bind next-k break-k final?-id clauses . _)
          ...
          [(_ orig-stx bindings+options . rest)
           (let ()
             ...
             (with-syntax ([([fold-var fold-init] ...) bindings]
               ...
               (with-syntax ([(int-var ...)
                              (map (lambda (fold-var)
                                     (datum->syntax fold-var (gensym (syntax-e fold-var))))
