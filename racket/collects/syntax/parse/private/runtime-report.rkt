...
;; prose-for-expects : (Listof Expect) Hash-Table -> String
(define (prose-for-expects expects config)
  ...
  (define what-vss (parition-by-what (append literal atom) (append literal-whats atom-whats)))
...
;; prose-for-expect : Hash-Table -> Expect -> String
...
(define (parition-by-what vs whats)
  (for/fold ([rev-vs #hash()]
             [rev-whats '()]
             #:result (for/list ([what (in-list (reverse rev-whats))])
                        (cons what (reverse (hash-ref rev-vs what)))))
            ([v (in-list vs)]
             [what (in-list whats)])
    (define l (hash-ref rev-vs what #f))
    (values (hash-update rev-vs what (lambda (vs) (cons v vs)) (or l '()))
            (if l rev-whats (cons what rev-whats)))))
