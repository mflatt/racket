...
(define (release-all)
  ;; Run on place exit, so in atomic mode
  ;; REMOVEME
  #;
  (let ([dses (let loop ([ds all-nodes])
                (cond
                  [ds (cons ds (loop (node-next ds)))]
                  [else null]))])
    (for ([ds (in-list dses)]) (remove-node! ds))
    (for ([ds (in-list dses)]) (add-node! ds)))
  (define ds all-nodes)
  (when ds
